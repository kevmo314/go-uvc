...
func main() {
	...
	// Debug: print all interfaces found
	for i, si := range info.StreamingInterfaces {
		fmt.Printf("Interface %d: num=%d, alt=%d, channels=%d, bits=%d, freqs=%v, endpoint=0x%02x\n",
			i, si.InterfaceNumber(), si.AlternateSetting(),
			si.NrChannels, si.BitResolution, si.SamplingFreqs, si.EndpointAddress)
	}
