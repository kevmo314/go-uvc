...
func NewUVCDevice(fd uintptr) (*UVCDevice, error) {
	...
	return &UVCDevice{device: device}, nil
}
	...
	for i := 0; i != len(vcbuf); i += int(vcbuf[i]) {
		...
		case *descriptors.HeaderDescriptor:
			...
			for _, i := range ci.VideoStreamingInterfaceIndexes {
				...
				asi := &StreamingInterface{usb: &configDesc.Interfaces[i], bcdUVC: info.bcdUVC}
...
// control_transfer
func (si *StreamingInterface) ClaimFrameReader(uvc *UVCDevice, bcdUVC uint16, formatIndex, frameIndex uint8) (*FrameReader, error) {
